...
static void IsTrue(bool condition, const wchar_t *, ...)
{
...
{
    VERIFY_IS_TRUE(condition == false);
}
...
{
    {
        ...
        auto t2 = t1.then([=](int i) -> float {
            IsTrue(i == 47, L"Continuation did not recieve the correct value from ancestor. Expected: 47, Actual: %d", i);
            return (float)i/2;
        });

        float t2Result = t2.get();
        IsTrue(t2Result == 23.5, L"Continuation task did not produce the correct result. Expected: 23.5, Actual: %f", t2Result);

        task_status t2Status = t2.wait();
        IsTrue(t2Status == completed, L"Continuation task was not in completed state. Expected: %d, Actual: %d", completed, t2Status);
...
TEST(TestTasks_void_tasks)
{
    // Test void tasks
    {
        ...
        auto t2 = t1.then([&]() {
            IsTrue(value == 147, L"void continuation did not recieve the correct value from ancestor. Expected: 147, Actual: %d", value);
            value++;
        });
...
TEST(TestInlineChunker)
{
    ...
    volatile int sum = 0;
    async_for(0, 1, numiter, [&](int) -> task<void>
    {
        sum++;
        return create_task([]() {});
    }).wait();
...
TEST(PPL_Conversions_Nested)
{
    ...
    pplx::task<int> t2 = pplx::concurrency_task_to_pplx_task(pplx::pplx_task_to_concurrency_task(pplx::concurrency_task_to_pplx_task(pplx::pplx_task_to_concurrency_task(t1))));
    int n = t2.get();
